		...
		while (true) {
			try {
				...
				while (Constant.gpsList.size() > 0) {
					...
					/*
					 * String
					 * loc=ChangeLonLat.getQuickMapLocation(location.getLon(), location.getLat()); 
					 * String[] locs=loc.split(",");
					 * if(locs.length>1){ isGX=true; } location.setLoc(locs[0]);
					 */
					...
					if (overSpeed != null) {
						...
						if (isChaosu) {
							if (!Constant.overSpeed.containsKey(location.getAssetid())) {
								...
								if (location.getGpstime() - alarmLocation.getGpstime() > Constant.OverSpeedAlarm
										&& alarmLocation.getAlarmId(Constant.OVERSPEEDALARM + "") < 1) {
					...
					if (alarmParams != null) {
						for (int i = 0; i < alarmParams.size(); i++) {
									...
									if (dis < Constant.MINPOINTDIS) { // 进入关键点
										if (!Constant.alarm.containsKey(key)) {
											...
											if (location.getGpstime() - alarmLocation.getGpstime() > Constant.GeoFenceAlarm
													&& alarmLocation.getAlarmId(alarm.getAlarmtype() + "") < 1) {
												long id = ConstantC.getUIdL();
												MongoDBOpe db = new MongoDBOpe();
												Map param = new HashMap();
							...
							case Constant.INOUTALARM:
								...
								if (curGeo != null && keyPoint != null) {
									boolean isIn = false;
									if (alarm.getGeotype() == Constant.MAPPOLYGON
											|| alarm.getGeotype() == Constant.MAPRECT) {
									...
									if (isIn) { // 在geofence
										if (alarm.getAlarmtype() == Constant.INALARM
												|| alarm.getAlarmtype() == Constant.INOUTALARM) { // 驶入报警开始
											if (!Constant.alarm.containsKey(key)) {
												Long l = Constant.removeAlarm.get(key);
												if (l == null) {
													l = 0L;
										...
										if (alarm.getAlarmtype() == Constant.OUTALARM
												|| alarm.getAlarmtype() == Constant.INOUTALARM) { // 驶出报警结束
										...
										if (alarm.getAlarmtype() == Constant.OUTALARM
												|| alarm.getAlarmtype() == Constant.INOUTALARM) { // 驶出报警开始
											if (!Constant.outAlarm.containsKey(key)) {
										...
										if (alarm.getAlarmtype() == Constant.INALARM
												|| alarm.getAlarmtype() == Constant.INOUTALARM) { // 驶入报警结束
											if (Constant.alarm.containsKey(key)) {
												GPSLocation alarmLocation = Constant.alarm.remove(key);
												if (location.getGpstime() - alarmLocation.getGpstime() > Constant.GeoFenceAlarm) {
													if (alarmLocation.getAlarmId(alarm.getAlarmtype() + "") < 1) {
